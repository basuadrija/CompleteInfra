...
resource "aws_eks_cluster" "newtest" {
  name     = "newtest-cluster"
  role_arn = aws_iam_role.newtestrole.arn
...
resource "aws_eks_addon" "ebs_csi_driver" {
  cluster_name    = aws_eks_cluster.newtest.name
...
resource "aws_eks_node_group" "newtestng" {
  cluster_name    = aws_eks_cluster.newtest.name
  node_group_name = "newtestng"
  node_role_arn   = aws_iam_role.newtestrole.arn
  subnet_ids      = aws_subnet.test_subnet[*].id
...
resource "aws_iam_role" "newtestrole" {
  name = "newtestrole"
...
resource "aws_iam_role_policy_attachment" "newtestrole_policy" {
  role       = aws_iam_role.newtestrole.name
